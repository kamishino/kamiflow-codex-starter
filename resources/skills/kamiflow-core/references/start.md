...
- Balanced
- Ambitious
8. Score each track on `1-5`:
- impact
- feasibility
- effort (5 is easiest/fastest)
- confidence
9. Compute total `/20` and label:
- Go: 16-20
- Maybe: 11-15
- Kill: <=10
10. Pick one **Best Solution** and provide rationale with key tradeoffs.
11. Add a pre-mortem for Best Solution.
12. For complex or architecture-sensitive requests, include a one-line Design Surface note before handoff and avoid hard design commitments on the same turn.
13. Produce `START_CONTEXT` block for `plan` handoff:
- `topic`
- `target_user`
- `success_30d`
- `constraints`
...
14. Emit one exact `Run next:` command for `plan`.
...
15. Produce `Start Summary` fields for plan persistence:
- `Required: yes|no`
- `Reason`
- `Selected Idea`
...
- `Pre-mortem Risk`
- `Handoff Confidence`
16. Run Diagram Need Decision immediately after user answers (or IDEATION_CONTEXT ingestion):
...
17. Persist `diagram_mode` in target plan markdown and apply policy:
...
18. End with one handoff route: `plan`, `build`, or `research`.
19. Include concise next-step guidance when useful; do not require verbose response footer fields.
20. Persist direct plan-file mutation before final output:
