...
`kfc client` is the reusable client-project entrypoint. It starts with an inline inspection pass, auto-initializes a minimal `package.json` when the target folder is truly empty, keeps `kamiflow.config.json` optional by default, creates or refreshes the root `AGENTS.md` managed contract, installs the project-local runtime skill at `.agents/skills/kamiflow-core/SKILL.md`, syncs `.codex/rules/kamiflow.rules`, creates the private project-local Codex binding at `.codex/config.toml`, creates or refreshes `.kfc/CODEX_READY.md`, scaffolds private client lessons at `.kfc/LESSONS.md` plus `.local/kfc-lessons/`, ensures `.gitignore` contains `.kfc/`, `.local/`, `.agents/`, and `.codex/config.toml`, recovers a missing active plan automatically, runs one smart-recovery cycle only for recoverable bootstrap issues, and auto-launches Codex when a real mission is available:
...
- if no real mission is available yet, bootstrap still ends in PASS, preserves `.kfc/CODEX_READY.md`, skips auto-launch, and points you to `kfc client --goal "<goal>"`
...
Root `AGENTS.md` is the stable client-repo brain. KFC owns and refreshes its managed block as the project-specific `/init` contract. It now includes the client workflow command map for `kfc client`, `kfc client status`, `kfc plan validate`, `kfc flow ensure-plan`, `kfc flow ready`, `kfc client doctor --fix`, and `kfc client done`, plus portable Kami Flow Core sections for `Plan Lifecycle Contract`, `Evidence Gate`, `Smooth Flow Protocol`, `Markdown Readability Policy`, blocker recovery, and docs/closeout review. Read `.kfc/CODEX_READY.md` when present; otherwise recover one active non-done plan with `kfc flow ensure-plan --project .` and continue from the active plan plus lessons. Project-root auto-detection is shared across these KFC entrypoints, so add `--project <path>` only when you intentionally target a project from outside its tree.
Repo-built KFC also resolves packaged client onboarding resources from the repository root even when the command is executing from compiled `dist` entrypoints, so generated AGENTS/ready-brief artifacts do not depend on a `dist/package.json` mirror.
When KFC prints an apply/recovery command for an out-of-tree client target, keep the emitted absolute `--project <path>` intact instead of replacing it with `.`.
