...
| AP-007 | plan | Skip readiness gate before implementation | `build`/`fix` starts without `kfc flow ready --project .` success | `kfc flow ensure-plan --project .` then `kfc flow ready --project .`; if blocked, switch to `$kamiflow-core plan` | `scripts/policy/verify-codex-intelligence-contract.mjs` | `resources/skills/kamiflow-core/references/build.md`, `resources/skills/kamiflow-core/references/fix.md` |
