...
- `kfc-plan` is package-internal (`@kamishino/kfc-plan-web`) and delegated by `kfc plan`.
...
- Use `KFC_PLAN_BASE_URL` when set.
...
- For browser-facing KFC Plan UI changes, run `npm run kfc-plan:test:browser` in addition to the fast/package and design-system checks.
- Treat Playwright as a targeted KFC Plan UI gate, not a mandatory repo-wide test for non-UI work.

## Activity Signal Semantics

KFC Plan Activity panel is observer-first; use it as evidence, not control.
...
- Preferred control plane is chat-driven Codex execution; KFC Plan remains read/observe oriented.
- Mermaid/flow diagrams in KFC Plan should be shown in a dedicated **Technical Solution Diagram** section, not embedded inside PlanSnapshot status visualization.
...
- KFC Plan may render a minimal derived placeholder when the section/block is missing, but canonical solution intent must come from plan markdown.
- KFC Plan attempts pan/zoom controls via `svg-pan-zoom`; if unavailable, it falls back to static Mermaid rendering.
...
- Missing `.local/`: create `.local/plans/` and bootstrap from `packages/kfc-plan-web/templates/plan-template.md`.
...
- `request_user_input` unavailable in a Codex action run: use `mode_hint: Plan` (or include `request_user_input` in the action prompt). KFC Plan now auto-tries Plan-profile/config variants first, then falls back to `codex exec -` if those flags are unsupported.
