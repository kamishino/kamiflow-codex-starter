...
That managed block should also be the first place Codex learns the client-project workflow command map: `kfc client`, `kfc client status --project .`, `kfc plan validate --project .`, `kfc flow ensure-plan --project .`, `kfc flow ready --project .`, `kfc client doctor --project . --fix`, and `kfc client done`.
...
- Use `kfc client status --project .` when you need a read-only repo health + next-action snapshot before deciding whether bootstrap, doctor, or update is necessary.
