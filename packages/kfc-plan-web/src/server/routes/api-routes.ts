...
export function registerApiRoutes(fastify: any, deps: any): void {
  ...
  function codexControlDisabledPayload(methodPath: string) {
    return {
      error: "Direct Codex execution from KFC Plan is disabled. Use Codex chat as the control plane.",
      error_code: "CODEX_ACTION_DISABLED",
      method_path: methodPath,
      recovery: "Run Codex from chat and use KFC Plan Activity Stream as observer evidence."
    };
  }
