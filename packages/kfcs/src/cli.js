...
import { createKfcsServer } from "./server.js";

function usage() {
  console.log(`KFCS

Usage:
  kfcs <command> [options]

Commands:
  serve    Start the local session-manager web app
  index    Print a summary of the current Codex sessions root
  where    Print the Codex sessions root used by KFCS
...
async function runServe(parsed) {
  const server = await createKfcsServer({
  ...
  console.log(`KFCS listening at ${listener.url}`);
...
async function runFind(parsed) {
  if (!parsed.id) {
    throw new Error("Missing --id for `kfcs find`.");
  }
...
async function runExport(parsed) {
  if (!parsed.id || !parsed.to) {
    throw new Error("Missing --id or --to for `kfcs export`.");
  }
...
async function runImport(parsed) {
  if (!parsed.from) {
    throw new Error("Missing --from for `kfcs import`.");
  }
...
async function runRestore(parsed) {
  if (!parsed.id) {
    throw new Error("Missing --id for `kfcs restore`.");
  }
...
export async function runCli(argv) {
  ...
  try {
    ...
    if (parsed.command === "detail") {
      if (!parsed.id) {
        throw new Error("Missing --id for `kfcs detail`.");
      }
