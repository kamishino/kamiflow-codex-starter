...
import { createKfcsServer } from "../src/server.js";
...
async function runCase(name, fn) {
  try {
    await fn();
    console.log(`[kfcs:test] PASS ${name}`);
  } catch (err) {
    failed += 1;
    console.error(`[kfcs:test] FAIL ${name}`);
    console.error(`  ${err instanceof Error ? err.message : String(err)}`);
  }
}

async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kfcs-"));
  try {
    await fn(tempDir);
  } finally {
...
await runCase("server exposes health, list, detail, export, import, and restore", async () => {
  await withTempDir(async (tempDir) => {
    ...
    await seedSessions(tempDir);
    const server = await createKfcsServer({ sessionsRoot: tempDir, host: "127.0.0.1", port: 0 });
    await server.ready();
    ...
    assert.ok(html.payload.includes("Codex Session Manager"));
    assert.ok(html.payload.includes("Transcript Tail"));

    const script = await server.fastify.inject({ method: "GET", url: "/assets/kfcs.js" });
    assert.equal(script.statusCode, 200);
    assert.ok(script.payload.includes("copy-id-button"));
    assert.ok(script.payload.includes("/api/sessions/export"));

    const styles = await server.fastify.inject({ method: "GET", url: "/assets/kfcs.css" });
...
if (failed > 0) {
  process.exitCode = 1;
  console.error(`[kfcs:test] ${failed} test(s) failed.`);
} else {
  console.log("[kfcs:test] all tests passed.");
}
