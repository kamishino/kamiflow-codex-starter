...
## Start Summary
- Required: no
- Reason: Browser smoke fixture for KFP.
...
\`\`\`mermaid
flowchart LR
  A[fixture plan] --> B[KFP browser smoke]
  B --> C[plan picker]
  B --> D[theme]
  B --> E[phase timeline]
\`\`\`
...
- ${definition.updatedAt} - Next step: ${definition.wipNext ?? "Open KFP browser smoke."}
`;
}

async function createProjectFixture(options = {}) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kfp-browser-"));
