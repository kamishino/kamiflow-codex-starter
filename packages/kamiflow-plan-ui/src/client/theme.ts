...
const THEME_STORAGE_KEY = "kfp.theme.preference.v1";
...
function desktopBridge(): DesktopThemeBridge | null {
  const maybeWindow = window as typeof window & { kfpDesktopTheme?: DesktopThemeBridge };
  if (!maybeWindow.kfpDesktopTheme?.isDesktop) {
    return null;
  }
  return maybeWindow.kfpDesktopTheme;
}
