...
import { buildHtml, KFC_SESSION_CSS, KFC_SESSION_JS } from "./ui.js";
...
export async function createKfcSessionServer(options = {}) {
  ...
  fastify.get("/assets/kfc-session.css", async (_request, reply) => {
    return reply.type("text/css; charset=utf-8").send(KFC_SESSION_CSS);
  });

  fastify.get("/assets/kfc-session.js", async (_request, reply) => {
    return reply.type("application/javascript; charset=utf-8").send(KFC_SESSION_JS);
  });
