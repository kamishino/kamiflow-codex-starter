...
import { createKfcsServer } from "../../packages/kfcs/src/server.js";
import { writeFixtureSession } from "../../packages/kfcs/src/session-store.js";

const TEMP_DIR = await fs.mkdtemp(path.join(os.tmpdir(), "kfcs-smoke-"));
...
async function main() {
  ...
  try {
    ...
    assert.ok(pageText.includes("Codex Session Manager"));
    assert.ok(pageText.includes("Export Session"));

    console.log("[kfcs-smoke] PASS");
  } finally {
    await server.close();
